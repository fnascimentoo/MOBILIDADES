...
function atualizarMapa() {
  ...
  const promessas = [
    ...
    ...variaveisSelecionadas.map(v => {
      if (configuracao === "correlacao") {
        const anoFinal = periodo.split("_")[1];
        return d3.json(`data/${v}_CORR_${anoFinal}.json`);
      } else {
        return d3.json(`data/${v}_${periodo}.json`);
      }
    })
  ];
  ...
  Promise.all(promessas).then(([topoMun, topoUF, ...camadas]) => {
    ...
    desenharLegenda(configuracao);
  });
}

function desenharLegenda(configuracao) {
  const container = d3.select("#legenda");
  container.selectAll("*").remove();

  if (configuracao === "correlacao") {
    const categorias = Object.entries(categoriaCoresCorrelacao);
    categorias.forEach(([categoria, cor]) => {
